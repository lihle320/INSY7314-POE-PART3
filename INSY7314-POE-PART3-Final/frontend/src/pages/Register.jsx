...
//api client handles communcation with backend
const api = {
    post: async (url, data) => {
        // fetch JWT for authenticated requests  (Web Dev Cody ,2025)
      const token = localStorage.getItem('token'); 
        
        const headers = {
            'Content-Type': 'application/json',
        };
// If token exists, add authorization header for protected endpoints
        if (token) {
...
//input validation regex patterns (Web Dev Simplified ,2019)
const regex = { 
...
export default function Register(){
  ...
  //gonna set sates based on values that are based on whether they meet the condition
...
//validates input fields using regex patterns
  ...
  const submit = async (e) => {
    ...
    // Prepare to send data to backend and trim whitespace from all fields
    try {
        const backendData = {
        ...
        //f backend returns JWT token, store it in loocally (Web Dev Cody ,2025)
        if (response.data.token) {
            localStorage.setItem('token', response.data.token);
        }
        //registeration compelted successfully
